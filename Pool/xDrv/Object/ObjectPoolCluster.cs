...
{
    /// <summary>
    /// 다수의 상이한 오브젝트를 제어하는 풀을 키 단위로 구분하여 보유하는 클래스
    /// </summary>
    public class ObjectPoolCluster<Key, CreateParams, ActivateParams> : IObjectPoolCluster<Key, CreateParams, ActivateParams>
        ...
        where ActivateParams : IObjectActivateParams
    {
        ...
        public ObjectPoolCluster(Func<Key, bool> p_IsAvailablePop = null, Action<Key> p_OnPoolAdd = null, Action<Key> p_OnPoolRemove = null)
        {
        ...
        /// </summary>
        public Return Pop<Return>(Key p_Key, ActivateParams p_ActivateParams) where Return : ObjectPoolContent<Return, CreateParams, ActivateParams>, new()
        {
            if(IsAvailablePop?.Invoke(p_Key) ?? true)
            {
                var targetPool = _poolCluster[p_Key] as ObjectPool<Return, CreateParams, ActivateParams>;
