...
namespace k514
{
    ...
    /// </summary>
    public abstract class EditorSingleton<Me> : EditorSingletonBase<Me> where Me : EditorSingleton<Me>
    {
        #region <Consts>
        ...
        /// 싱글톤 생성 및 초기화 메서드
        /// </summary>
        private static Me GetInstanceSafe(Me p_Target)
        {
            switch (_CurrentSingletonPhase)
            {
                case SingletonTool.SingletonInitializePhase.None:
                {
                    if (SystemBoot.IsSingletonAvailable)
                    {
                        try
                        {
                            CreateSingletonInstance(p_Target);
                        }
#if APPLY_PRINT_LOG
                        catch(Exception e)
                        {
        ...
        /// 싱글톤 초기화 메서드
        /// </summary>
        private static void CreateSingletonInstance(Me p_Target)
        {
